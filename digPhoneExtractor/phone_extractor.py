...
# @Last Modified time: 2016-09-30 13:33:41
...
class ZEPreprocessor():
    ...
    def prep_datetime(self, raw):
        m = ZEPreprocessor.re_datetime_regex.findall(raw)
        for d in m:
            dd = ''.join(ZEPreprocessor.re_digits_regex.findall(d))
    ...
    def preprocess(self, raw):
        ...
        raw = ZEPreprocessor.re_prep.sub(' ', raw)
        raw = ZEPreprocessor.re_all_regex.sub('', raw)
        raw = ZEPreprocessor.re_simple_format.sub('pnwrapper \g<0> pnwrapper', raw)
        return raw
...
class ZETokenizer():
    ...
    def tokenize_url(self, raw):
        ...
        # parse path
        path = url_obj.path
        path = ZETokenizer.re_2_digts_only_in_url_regex.sub('', path)
        path = path.split('/')

        content = netloc + path

        content = [SEPARATOR.join(ZETokenizer.re_all_alphabet_in_url_regex.findall(_)) for _ in content]
...
class ZECleaner():
    ...
    def prep_replace_numeral_words(self, raw):
        ...
        raw = ZECleaner.re_twenty_x.sub("2", raw)
        raw = ZECleaner.re_thirty_x.sub("3", raw)
        raw = ZECleaner.re_forty_x.sub("4", raw)
        raw = ZECleaner.re_fifty_x.sub("5", raw)
        raw = ZECleaner.re_sixty_x.sub("6", raw)
        raw = ZECleaner.re_seventy_x.sub("7", raw)
        raw = ZECleaner.re_eighty_x.sub("8", raw)
        raw = ZECleaner.re_ninety_x.sub("9", raw)

        raw = ZECleaner.re_ten.sub("10", raw)
        raw = ZECleaner.re_one.sub("1", raw)
        raw = ZECleaner.re_zero.sub("0", raw)
...
class ZEValidator():
    ...
    def is_all_dup_digits(self, raw):
        for i in range(1, 6):
            rtn = ZEValidator.re_num_digits[i].findall(raw)
            if len(raw) % i != 0:
                continue
    ...
    def validate(self, raw):
        ...
        for nums in raw.split('\t'):
            nums = nums.strip()
            nums = ZEValidator.re_start_zero.sub('', nums)

            if len(nums) > 16:
                continue

            if len(ZEValidator.re_zero.findall(nums)):
                continue
...
class ZENormalizer():
    ...
    def normalize(self, cleaned_output, uncleaned_output, output_format='list'):
        # print [_.strip() for _ in ZENormalizer.re_digits.findall(tokenized_content) if _.strip() != '']
...
class PhoneNumberExtractor(object):
    ...
    def __init__(self, _output_format='list'):
        ...
        self.tokenizer = ZETokenizer(source_type='text')
        self.extractor = ZEExtractor()
        self.cleaner = ZECleaner()
        self.validator = ZEValidator()
...
class PhoneExtractor(Extractor):
    ...
    def __init__(self):
        self.renamed_input_fields = 'text' #['url', 'raw_content']  # ? renamed_input_fields
    ...
    def extract(self, doc):
        ...
        # urls = URLExtractor.extract(doc['url'])
        # extractor = PhoneNumberExtractor(doc['raw_content'])
